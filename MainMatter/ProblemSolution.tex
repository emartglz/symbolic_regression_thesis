          ...
          \begin{center}
              \begin{adjustbox}{width=0.4\textwidth, keepaspectratio}
                  \begin{tikzpicture}[
                      ...
                      \node[roundnode]        (star_2_1)    [below=of plus_2]    {$*$};
                      \node[squarednode]      (a_2)      [below=of star_2_1]    {$a_2$};
                      \node[squarednode_yellow]      (I_star_2_1)      [below right=of star_2_1]   {$y_2$};
...
Con las operaciones de mutación y cruzamiento solo faltaría detallar la operación de selección para poder finalmente implementar un algoritmo genético. Los detalles de esta última operación se ven en la siguiente sección.


\section{Determinación de las soluciones que pasan a la siguiente generación}

Dada una población inicial en una generación, se selecciona un subconjunto de individuos de esta y se mutan, y se toma otro subconjunto de individuos y se cruzan entre ellos. Luego estos nuevo subconjuntos se suman a la población inicial de la generación formando un nuevo conjunto de individuos el cual se define como población total de la generación.

De esta población total de la generación se seleccionan los sistemas que mejor ajustan los datos. Son seleccionados también algunos individuos aleatorios con el fin de no caer en mínimos locales en la búsqueda del mejor sistema. La cantidad de soluciones seleccionadas en este proceso es igual a la cantidad presente en la población inicial.

Como resultado de esta selección se obtiene una nueva población la cuál será la población inicial para la siguiente generación. Estas operaciones de mutación y cruzamiento de la población inicial y luego selección de individuos se pueden repetir múltiples veces con el fin de realizar numerosas generaciones obteniendo mejores soluciones cada vez.


Una vez definida la forma de representar una solución como un árbol computacional, se pueden definir las operaciones de mutación y cruzamiento sobre estos árboles. Conociendo la forma de determinar que tan cercanos son los puntos obtenidos de una solución con respecto a los puntos originales se puede crear un proceso de selección de individuos. Con estas operaciones definidas se puede implementar un algoritmo genético y con este algoritmo se puede definir una regresión simbólica con el fin de obtener el sistema de ecuaciones diferenciales lineales con respecto a los parámetros que mejor ajuste un conjunto de datos.

En el siguiente capítulo se muestran los resultados obtenidos al utilizar la regresión simbólica planteada para encontrar los sistemas de ecuaciones difenciales originales de varios conjuntos de puntos.